<p align="center">
  <img src="../../img/logo-128.png" alt="TrekReel Logo" width="120" height="120">
</p>

<h1 align="center">TrekReel</h1>

<p align="center">
  <strong>Transformez chaque itinéraire extérieur Dans une histoire 3D partageable</strong><br>
  Conçu pour les randonneurs, les coureurs et les cyclistes. Importez GPX, créez instantanément des vidéos cinématographiques 3D.
</p>

<p align="center">
  <a href="https://hooosberg.github.io/TrekReel/">
    <img src="https://img.shields.io/badge/Website-TrekReel-0a66c2?style=flat-square" alt="Website">
  </a>
  <a href="https://github.com/hooosberg/TrekReel/releases">
    <img src="https://img.shields.io/github/v/release/hooosberg/TrekReel?style=flat-square" alt="Release">
  </a>
  <a href="https://github.com/hooosberg/TrekReel/blob/main/LICENSE">
    <img src="https://img.shields.io/github/license/hooosberg/TrekReel?style=flat-square" alt="License">
  </a>
  <img src="https://img.shields.io/badge/Platform-macOS-111111?style=flat-square" alt="Platform">
</p>

<p align="center">
  <img src="https://img.shields.io/badge/GPS-GPX%20%7C%20KML%20%7C%20KMZ-2ea44f?style=flat-square" alt="Supported GPS Formats">
  <img src="https://img.shields.io/badge/Video%20Export-WebM%20%28VP9%29%20%7C%20MP4%20%28H.264%29-007aff?style=flat-square" alt="Supported Export Video Formats">
</p>

<p align="center">
  <strong>
    <a href="../../README.md">English</a> &nbsp;|&nbsp;
    <a href="./README.zh-CN.md">中文 (Simplified Chinese)</a> &nbsp;|&nbsp;
    <a href="./README.es-ES.md">Español</a> &nbsp;|&nbsp;
    Français &nbsp;|&nbsp;
    <a href="./README.de-DE.md">Deutsch</a> &nbsp;|&nbsp;
    <a href="./README.it-IT.md">Italiano</a> &nbsp;|&nbsp;
    <a href="./README.pt-BR.md">Português (Brasil)</a> &nbsp;|&nbsp;
    <a href="./README.ja-JP.md">日本語</a> &nbsp;|&nbsp;
    <a href="./README.ko-KR.md">한국어</a> &nbsp;|&nbsp;
    <a href="./README.ru-RU.md">Русский</a> &nbsp;|&nbsp;
    <a href="./README.ar-SA.md">العربية</a> &nbsp;|&nbsp;
    <a href="./README.hi-IN.md">हिन्दी</a>
  </strong>
</p>

---

### About

Conçu pour les randonneurs, les coureurs et les cyclistes. Importez GPX, créez instantanément des vidéos cinématographiques 3D.

### ✨ Highlights

#### 1. Aussi simple qu'une carte PPT
Générez automatiquement un terrain 3D à partir de GPX. Dessinez des itinéraires manuellement ou importez-les pour visualiser instantanément les plans.

![Route Animation](../../img/feature_route.jpg)

#### 2. Annotations riches
Ajoutez des marqueurs aux endroits clés. Visualisez les camps, les sources d'eau et les risques avec le contexte du terrain.

![Rich Annotations](../../img/feature_marker.jpg)

### 🚀 Features

| | |
| --- | --- |
| 🏔️ **Visualisation du terrain 3D** | Les données d'altitude réelles rencontrent l'imagerie satellite. visualisez votre chemin avec profondeur. |
| 📍 **Conte d'itinéraire** | Plus que la lecture. Ajoutez des images clés, des marqueurs photo et des notes pour raconter l'histoire. |
| 🎬 **Caméra cinématique** | Les outils d'interpolation fluides de la caméra donnent à vos vidéos d'itinéraire l'apparence de films. |
| 🔒 **La confidentialité d'abord, prêt hors ligne** |  |

---

### 📖 Resources

- **[Accueil](https://hooosberg.github.io/TrekReel/)**
- **[Assistance](https://hooosberg.github.io/TrekReel/support.html)**
- **[Politique de confidentialité](https://hooosberg.github.io/TrekReel/privacy.html)**
- **[Conditions d'utilisation](https://hooosberg.github.io/TrekReel/terms.html)**
- **[Crédits](https://hooosberg.github.io/TrekReel/sources.html)**

### 📬 Contact

- **GitHub**: [hooosberg/TrekReel](https://github.com/hooosberg/TrekReel)
- **Email**: [zikedece@proton.me](mailto:zikedece@proton.me)

### 🙏 Plus de projets

<table>
  <tr>
    <td align="center">
      <a href="https://hooosberg.github.io/WitNote/">
        <img src="../../img/witnote.png" width="80" alt="WitNote"><br>
        <b>WitNote</b>
      </a>
    </td>
    <td align="center">
      <a href="https://hooosberg.github.io/GlotShot/">
        <img src="../../img/glotshot-128.png" width="80" alt="GlotShot"><br>
        <b>GlotShot</b>
      </a>
    </td>
    <td align="center">
      <a href="https://uixskills.com/">
        <img src="../../img/uixskills.png" width="80" alt="UIX Skills"><br>
        <b>UIX Skills</b>
      </a>
    </td>
  </tr>
</table>

Copyright © 2026 TrekReel. All rights reserved.\n